...
function App() {
  ...
  // Marriage proposal state
  const [marriageNoLabel, setMarriageNoLabel] = useState("NO 💔");
  const [marriageHoveredOnce, setMarriageHoveredOnce] = useState(false);
  ...
  // Marriage proposal handlers
  const handleMarriageNoEnter = useCallback(() => {
    if (!marriageHoveredOnce) {
      setMarriageHoveredOnce(true);
    } else {
      setMarriageNoLabel("YESSS ❤️");
    }
  }, [marriageHoveredOnce]);

  const handleMarriageNoLeave = useCallback(() => {
    if (marriageHoveredOnce) {
      setMarriageNoLabel("NO 💔");
    }
  }, [marriageHoveredOnce]);

  const handleMarriageYes = useCallback(() => {
    setView("marriage-success");
  }, []);

  const handleMarriageNo = useCallback(() => {
    if (!marriageHoveredOnce) {
      setMarriageHoveredOnce(true);
    } else {
      handleMarriageYes();
    }
  }, [marriageHoveredOnce, handleMarriageYes]);
  ...
  // Effect to handle audio playback when song or playing state changes
  useEffect(() => {
    if (audioRef.current) {
      if (isPlaying) {
        const playPromise = audioRef.current.play();
    ...
    return (
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            <motion.img
              src={config.media.roseBearGif}
              alt="cute bear with rose"
            ...
            >
              ...
              <span className="ask"> {config.content.marriageTitle}</span>
            ...
            >
              {config.content.marriageSubtitle}
            </motion.p>
            ...
            >
              <motion.button
                className="btn yes"
                onClick={handleMarriageYes}
                whileHover={{
                  scale: 1.1,
                  boxShadow: "0 20px 40px rgba(255, 122, 162, 0.5)",
                }}
                whileTap={{ scale: 0.95 }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
              >
              ...
              <motion.button
                className="btn no"
                onMouseEnter={handleMarriageNoEnter}
                onMouseLeave={handleMarriageNoLeave}
                onClick={handleMarriageNo}
                whileHover={{
                  x: 15,
                  scale: 1.05,
                }}
  ...
  if (view === "marriage-success") {
    return (
      <div className="valentine-root success">
        <div className="card success-card">
          <h1 className="yay">{config.content.marriageSuccessMessage}</h1>
          <p className="subtitle small">{config.content.marriageSuccessSubtitle}</p>
